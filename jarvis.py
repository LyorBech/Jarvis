...
def assistant(command):
    ...
    if "I'm home" in command:
        ...
        time.sleep(3)
        muziekjes = myCommand().strip()
        if 'genre' in muziekjes:
            talkToMe('what genre?')
            genre = myCommand()
            url = 'https://open.spotify.com/view/' + genre + '-page'
            chrome_path = "C:/Program Files (x86)/Google/Chrome/Application/chrome.exe %s"
            webbrowser.get(chrome_path).open(url)
        elif 'my' in muziekjes:
        ...
        elif 'just' in muziekjes:
            url = 'https://open.spotify.com/browse/featured'
            chrome_path = "C:/Program Files (x86)/Google/Chrome/Application/chrome.exe %s"
            webbrowser.get(chrome_path).open(url)

        elif 'search' in muziekjes:
        ...
        system.open("Python Projects/keylogger.py")
        ...
        talkToMe("What would you like me to note")
        f = open(r'C:\Users\lyor\Documents\Projects\jarvis\notes.txt', 'a')
        note = myCommand()
        ...
        c = y.find("h1").find_all(text=True, recursive=False)
...
while listen == False:
    ini = myCommand()
    if 'Amy' in ini:
        talkToMe("What can I do for you?")
        time.sleep(1)
        assistant(myCommand())

    elif 'thank you' in ini:
        talkToMe('No problem ')
